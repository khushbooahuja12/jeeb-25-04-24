...
.table thead th {
                ...
                <div class="card m-b-30">
                        ...
                        <table class="table table-bordered dt-responsive nowrap">
                            <thead>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($order->getUser->id)
                                            <a href="/vendor/users/show/{{ $order->getUser->id }}">{{ $order->getUser->id }}</a>
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <th>Name</th>
                                    <td><a
                                            href="<?= url('vendor/users/show/'.$order->fk_user_id) ?>">{{ $order->getUser->name ?? 'N/A' }}</a>
                                    </td>
                                </tr>
                                <tr>
                                    <th>Email</th>
                                ...
                                @if ($order->getOrderProducts->where('is_out_of_stock', 0))
                                    @foreach ($order->getOrderProducts->where('is_out_of_stock', 0) as $key => $value)
                                        ...
                                        <tr class="{{$product_class}}">
                                            ...
                                            <td><a
                                                    href="<?= url('vendor/orders/product-detail/' . base64url_encode($value->id)) ?>"><i
                        ...
                        <h6>Out of stock items
                            @if ($order->getOrderProducts->where('is_out_of_stock', 1)->count())
                                @if ($order->getTechnicalSupportProduct->where('in_stock', 1)->count())
                                    ...
                                    @endif
                                @endif
                                {{-- <a href="<?//= url('vendor/orders/replacement_options') . '/' . $order->id ?>"
                                    ...
                                    @if ($replaced_products)
                                        @foreach ($replaced_products as $key => $value)
                                            <tr>
                                                ...
                                                <td><a
                                                        href="<?= url('vendor/orders/product-detail/' . base64url_encode($value->fk_order_id)) ?>" target="_blank">
                                                        <i class="fa fa-eye"></i>
                                                    </a>
                                                </td>
                                            </tr>
                    ...
                    <script>
                        function createTechnicalSupportTicket(order_id) {
                            $.ajax({
                                url: '<?= url('vendor/orders/create_customer_support_ticket') ?>',
                                type: 'POST',
                                data: {
                                    order_id: order_id
                                },
                    ...
                    @if ($order->status<=3)
                        <div class="card-body">
                            <form method="post" id="updateDriver"
                                action="{{ route('vendor.orders.update_driver', [base64url_encode($order->id)]) }}">
                                @csrf
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="form-group">
                            ...
                            <table class="table table-bordered dt-responsive nowrap">
                                <thead>
                                    <tr>
                                        <th>Name</th>
                                        <td><a
                                                href="<?= url('vendor/drivers/show/' . base64url_encode($order->getOrderDriver->getDriver->id)) ?>">{{ $order->getOrderDriver->getDriver->name ?? 'N/A' }}</a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
    ...
    <script>
        ...
        $("#refundForm").on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= url('vendor/orders/refund_order_amount') ?>',
                type: 'POST',
                data: $(this).serialize(),
                dataType: 'JSON',
                cache: false
        ...
        $("select[name=status]").on('change', function() {
            if ($(this).val() === '4') {
                $.ajax({
                    url: '<?= url('vendor/orders/cancel_order_detail') ?>',
                    type: 'POST',
        ...
        $("#updateStatus").on('submit', function(e) {
            ...
            if ($("select[name=status]").val()==4) {
                e.preventDefault();
                $.ajax({
                    url: '<?= url('vendor/orders/cancel_order') ?>',
                    type: 'POST',
                    data: $(this).serialize(),
                    dataType: 'JSON',
                    cache: false
