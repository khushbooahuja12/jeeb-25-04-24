    ...
    <div class="container-fluid">
        <div class="page-title-box">
            <div class="row align-items-center">
                <div class="col-sm-6">
                    <h4 class="page-title">Coupon List
                        <a href="<?= url('vendor/coupons/create') ?>" class="btn btn-primary">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a title="view details"
                                                    href="<?= url('vendor/coupons/show/' . base64url_encode($row->id)) ?>"><i
                                                        class="fa fa-eye"></i></a>&ensp;
                                                @if ($row->expiry_date > date('Y-m-d'))
                                                    <a title="view details"
                                                        href="<?= url('vendor/coupons/edit/' . base64url_encode($row->id)) ?>"><i
                                                            class="fa fa-edit"></i></a>&ensp;
                                                @endif
                                                @if ($row->expiry_date < date('Y-m-d')) 
                                                    &nbsp;&nbsp;<a
                                                        href="<?= url('vendor/coupons/recreate/' . base64url_encode($row->id)) ?>"
    ...
    <script>
        function changeCouponStatus(obj, id) {
            ...
            if (confirm_chk) {
                ...
                if (id) {
                    $.ajax({
                        url: "<?= url('vendor/coupons/change_coupon_status') ?>",
                        type: 'post',
