    ...
    <div class="container-fluid">
        <div class="page-title-box">
            <div class="row align-items-center">
                <div class="col-sm-6">
                    <h4 class="page-title">Driver Stores</h4><a class="btn btn-primary " href="{{route('fleet-store-list',['id' => 0])}}">Store Drivers</a>
                </div>
                <div class="col-sm-6">  
                    <br clear="all"/>
                            ...
                            <tbody>
                                @if ($drivers)
                                    @foreach ($drivers as $key => $row)
                                        <tr>
                                            <form class="collector_store" action="{{ route('fleet-update-driver-store') }}" method="POST" id="collector_store_{{ $row->id }}">
                                                @csrf
                                                ...
                                                <td style="width:13%;">
                                                    ...
                                                    <a href="{{ route('fleet-delete-driver-store',['id' => $row->id]) }}" onclick="return confirm('Are you sure you want to delete this?')" class="btn btn-danger">Delete</button>
                                                </td>
                                            </form>
                                        </tr>
                                    @endforeach
...
<script>
    ...
    $('.driver_save').on('click',function(e){
        ...
        if (id) {
            ...
            $.ajax({
                url: "<?= url('admin/fleet/store/assign_driver') ?>",
                type: 'post',
                data: formData,
                processData: false,
                contentType: false,
