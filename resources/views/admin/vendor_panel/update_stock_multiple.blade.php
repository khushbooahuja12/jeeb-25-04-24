...
<div class="page-body">
    ...
    <div class="container-fluid product-wrapper">
      <div class="product-grid">
          ...
          <div class="row">
                ...
                <div class="pro-filter-sec">
                  <div class="product-search">
                      <div class="form-group m-0">
                        <select class="form-control" name="fk_category_id" id="fk_category_id" onchange="getSubCategory(this);">
                          <option selected value="">--Select Category--</option>
                          @foreach ($categories as $category)
                            <option value="{{ $category->id }}" <?= $parent_category == $category->id ? 'selected' : '' ?>>{{ $category->category_name_en }}</option>
                          @endforeach
                        </select>
                      </div>
                  </div>
                      ...
                      <div class="form-group m-0">
                        <select name="fk_sub_category_id" id="fk_sub_category_id" data-title="Product Sub Category" class="form-control regInputs">
                            ...
                            @foreach ($sub_categories as $sub_category)
                              <option value="{{ $sub_category->id }}" {{ $subcategory == $sub_category->id ? 'selected' : '' }}>{{ $sub_category->category_name_en }}</option>
                            @endforeach
                        </select>
                      </div>
                  </div>
              ...
              <div class="col-md-3">
                <div class="pro-filter-sec">
                  <div class="product-search">
                      <div class="form-group m-0">
                        <input class="form-control" type="search" name="filter" id="filter" value="{{ $filter }}" placeholder="Search.." data-original-title="" title=""></button>
                      </div>
                  </div>
                </div>
              </div>
              ...
              <div class="col-md-1">
                <div class="pro-filter-sec">
                  <div class="product-search">
                      <div class="form-group m-0">
                        <a class="btn btn-warning" href="{{route('fleet-stock-update-base-products-panel',['id'=> base64url_encode($store->id),'category' => 0])}}">Clear</a>
                      </div>
                  </div>
                </div>
              </div>
        ...
        <ul class="nav nav-tabs border-tab" id="myTab" role="tablist">
          <li class="nav-item">
            <a href="{{route('fleet-stock-update-base-products-panel',['id'=> base64url_encode($store->id),'category' => 0])}}"  class="nav-link active" >Update Stock All Products</a>
          </li>
                    ...
                    <form name="form_<?= $row->product_store_id ?>" id="form_<?= $row->product_store_id ?>" enctype="multipart/form-data">
                      ...
                      <div class="mb-3 m-form__group">
                        <div class="input-group"><span class="input-group-text">PRICE</span>
                          <input class="form-control edit_price" type="text" name="product_store_distributor_price" value="{{ $row->product_store_distributor_price }}">
                        </div>
                      </div>
                      <div class="mb-3 m-form__group">
  ...
  <script>
    $('#searchCompanyForm').on('submit',function(e){
        ...
        let category = $('#fk_category_id').val();
        let subcategory = $('#fk_sub_category_id').val();
        
        window.location.href = '?fk_category_id='+category+'&fk_sub_category_id='+subcategory+'&filter='+filter; 
