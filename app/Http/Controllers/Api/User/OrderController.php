...
class OrderController extends CoreApiController
{
    ...
    protected function apply_coupon(Request $request)
    {
        try {
            ...
            $this->required_input($request->input(), ['coupon_code','delivery_time']);

            $coupon = Coupon::where(['coupon_code' => $request->input('coupon_code'), 'status' => 1, 'model' => $request->input('delivery_time')])->first();
            if (!$coupon) {
    ...
    protected function make_order(Request $request)
    {
        try {
            ...
            $tracking_id = add_tracking_data($this->user_id, 'make_order', $request, '');
            ...
            if ($paid) {
                ...
                if ($create) {
                    ...
                    // // Check store closed or not
                    // $delivery_time = $request->input('delivery_time');
                    // $later_time = $request->input('later_time');
                    // if(mktime(23, 0, 0) < time()) {
                    //     throw new Exception("Next day delivery will be processed the following day.", 105);
                    // } else {
                    //     throw new Exception("Will be processed today.", 105);
                    // }
                ...
                // Adding card products to order
                $cart_products = UserCart::where(['fk_user_id' => $this->user_id])->get();
                foreach ($cart_products as $key => $value) {

                    $store = Store::where(['id' => $create->fk_store_id, 'status' => 1, 'deleted' => 0])->first();
                    if (!$store) {
                        if (isset($tracking_id) && $tracking_id) {
                        ...
                        $error_message = $lang == 'ar' ? 'المتجر غير متوفر' : 'The store is not available';
                        throw new Exception($error_message, 105);
                    } elseif ( $value->getProduct->product_type=="product" && $store->company_id != $value->getProduct->fk_company_id) {
                        if (isset($tracking_id) && $tracking_id) {
                            update_tracking_response($tracking_id,'Products are not available.');
                        }
                    ...
                    $selling_price = number_format($value->getProduct->$store_price, 2);
                    ...
                    OrderProduct::create([
                        ...
                        'itemcode' => $value->getProduct->itemcode ?? '',
                        'barcode' => $value->getProduct->barcode ?? '',
                        'fk_category_id' => $value->total_getProduct->fk_category_id ?? '',
                        'category_name' => $value->getProduct->category_name ?? '',
                        'category_name_ar' => $value->getProduct->category_name_ar ?? '',
                        'fk_sub_category_id' => $value->getProduct->fk_sub_category_id ?? '',
                        'sub_category_name' => $value->getProduct->sub_category_name ?? '',
                        'sub_category_name_ar' => $value->getProduct->sub_category_name_ar ?? '',
                        'fk_brand_id' => $value->getProduct->fk_brand_id ?? '',
                        'brand_name' => $value->getProduct->brand_name ?? '',
                        'brand_name_ar' => $value->getProduct->brand_name_ar,
                        'product_name_en' => $value->getProduct->product_name_en ?? '',
                        'product_name_ar' => $value->getProduct->product_name_ar ?? '',
                        'product_image' => $value->getProduct->product_image ?? '',
                        'product_image_url' => $value->getProduct->product_image_url ?? '',
                        'margin' => $value->getProduct->margin ?? '',
                        'distributor_price' => $value->getProduct->distributor_price ?? '',
                        'unit' => $value->getProduct->unit ?? '',
                        '_tags' => $value->getProduct->_tags ?? '',
                        'tags_ar' => $value->getProduct->tags_ar ?? '',
                        'discount' => $value->total_discount,
                        'product_quantity' => $value->quantity,
                        'product_weight' => $value->getProduct->unit ?? '',
                        'product_unit' => $value->getProduct->unit ?? '',
                        'sub_products' => $value->sub_products ?? ''
                    ]);

                    // if ($delivery_time == 1 && $order_status > 0) {
                    if ($order_status > 0) {
                        $storekeeper = Storekeeper::join('stores', 'storekeepers.fk_store_id', '=', 'stores.id')
                            ...
                            ->where('storekeepers.fk_store_id', '=', $user->nearest_store)
                            ->where('storekeeper_sub_categories.fk_sub_category_id', '=', $value->getProduct->fk_sub_category_id)
                            ->first();
                        if ($storekeeper) {
    ...
    protected function get_subtotal_forgot_something_order(Request $request)
    {
        try {
            ...
            $order = Order::where(['id'=>$request->input('id'),'fk_user_id'=>$this->user_id])->first();
            if (!$order) {
                throw new Exception($lang == 'ar' ? "طلبك السابق غير نشط" : "Your previous order is not active", 105);
            }
            ...
            if ($request->input('product_json') != '') {
                ...
                foreach ($content_arr as $key => $value) {
                    ...
                    if (!$product || $product->fk_company_id != $company_id) {
                        $error_message = $lang == 'ar' ? 'المنتجات غير متوفرة' : 'Products are not available.';
                        throw new Exception($error_message, 105);
                    }
                    ...
                    if ($product->fk_company_id == env("RETAILMART_ID") && $product->$store > 0 && $product->$store < $value->quantity) {
                        $cartItemQuantityMismatch[$key] = [
                            ...
                            'product_total_price' => (string) number_format($product->$store_price, 2)*$value->quantity,
                        ...
                        $price_total_with_qunatity_mismatch += $product->$store_price * $product->$store;
                    }
                    // For other companies
                    elseif ($product->$store > 0) {
                        ...
                        // Calculate the price and amount
                        $price_total += $product->$store_price * $value->quantity;
                        $price_total_with_qunatity_mismatch += $product->$store_price * $value->quantity;
                    } else {
    ...
    protected function make_forgot_something_order(Request $request)
    {
        try {
            ...
            $tracking_id = add_tracking_data($this->user_id, 'make_forgot_something_order', $request, '');
            ...
            elseif ($parent_order->status>=2) {
                ...
                if ($request->input('paymentResponse') && $request->input('paymentResponse')!='') {
                    ...
                    if ($payment['status']=="success") {
                        ...
                        makeRefund('wallet', $this->user_id, null, $refund_amount, $paymentResponseArr);
                    }
                }
                throw new Exception($lang == 'ar' ? "تم بالفعل تعبئة طلبك السابق. الرجاء تقديم طلب جديد" : "Your previous order is already packed. Please make new order..", 105);
            }
                    ...
                    if ($create) {
                        ...
                        if ($request->input('product_json') != '') {
                            ...
                            foreach ($content_arr as $key => $value) {
                                ...
                                } elseif ($store->company_id != $product->fk_company_id) {
                                    $error_message = $lang == 'ar' ? 'المنتجات غير متوفرة' : 'Products are not available.';
                                    throw new Exception($error_message, 106);
                                } else {  
                                    $store_no = get_store_no($store->name);
                                ...
                                $selling_price = number_format($product->$store_price, 2);
    ...
    protected function get_subtotal_replacement_order(Request $request)
    {
        try {
            ...
            // Set price key and price total
            $product_price_key = 'store' . $store_no . '_price';
            $price_total = 0;
            ...
            if ($request->input('product_json') != '') {
                ...
                foreach ($content_arr as $key => $value) {
                    ...
                    // Calculate the price and amount
                    $price_total += number_format(($product->$product_price_key * $value->product_quantity), 2);
                }
        ...
        try {
            ...
            if ($paid) {
                ...
                if ($paid->status != "success" && $payment_type=='online') {
                    ...
                    // Refund happens when storekeeper mark it as out of stock
                    // if ($total_amount < $totalOutOfStockProduct) {
                    //     $refund_amount = $totalOutOfStockProduct - $total_amount;
                    //     makeRefund('wallet', $this->user_id, null, $refund_amount, $order->orderId);

                    //     $last_message = "Your product has been replaced. The extra money has been refunded to your wallet. Your Order will be delivered in " . getDeliveryIn($order->getOrderDeliverySlot->expected_eta ?? 0);
                    // }
    ...
    protected function order_detail(Request $request)
    {
        try {
            ...
            $order_products = OrderProduct::leftJoin($this->products_table, 'order_products.fk_product_id', '=', $this->products_table . '.id')
                ->select($this->products_table . '.*', 'order_products.product_unit', 'order_products.product_unit', 'order_products.product_image_url', 'order_products.product_weight', 'order_products.total_product_price', 'order_products.single_product_price', 'order_products.product_quantity', 'order_products.discount', 'order_products.is_out_of_stock', 'order_products.sub_products')
    ...
    protected function rate_order(Request $request)
    {
        try {
            ...
            $rating_arr = json_decode($rating, true);
            if($rating_arr){ 
                foreach ($rating_arr as $key => $value) {
                    $update = OrderReview::create([
                        ...
                        'products_quality' => $value['products_quality'],
                        'delivery_experience' => $value['delivery_experience'],
                        'overall_satisfaction' => $value['overall_satisfaction'],
        ...
        try {
            ...
            if ($order->getOrderProducts->count()) {
                foreach ($order->getOrderProducts as $key => $value) {
                    ...
                    // If products are not belongs to the company
                    if ($product->fk_company_id != $company_id) {
                        UserCart::where(['fk_user_id' => $this->user_id])->delete();       
                        $error_message = $lang == 'ar' ? 'المتجر ليس بالقرب من عنوانك' : 'The store is not nearby your address';
                        throw new Exception($error_message, 106);
                    }
                    $selling_price = number_format($value->getProduct->$product_price_key, 2);

                    $exist = UserCart::where(['fk_product_id' => $value->fk_product_id, 'fk_user_id' => $this->user_id])->first();
