...
class ProductController extends CoreApiController
{
    ...
    protected function view_all_favorite_products(Request $request)
    {
        try {
            ...
            $products = Product::leftJoin('categories', $this->products_table . '.fk_category_id', '=', 'categories.id')
                ->leftJoin('brands', $this->products_table . '.fk_brand_id', '=', 'brands.id')
                ->join('user_wishlist', $this->products_table . '.id', '=', 'user_wishlist.fk_product_id')
                ->select(
                    $this->products_table . '.*',
                    'categories.id as category_id',
                    'categories.category_name_en',
