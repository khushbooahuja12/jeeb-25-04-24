    ...
    public function handle()
    {
        ...
        if (!$stocks) {
            ...
            foreach ($stocks as $stock) {
                ...
                } elseif ($itemCodeExist_without_0) {
                ...
                if($selected_record){
                    ...
                    \Log::info('Batch_id: '.$this->key.', Checking stock id: '.$stock->id.', with the itemcode '.$itemcode_without_0);
                    \Log::info('Batch_id: '.$this->key.', Checking stock id: '.$selected_record->id.', with the itemcode '.$selected_record->itemcode.', with the barcode '.$selected_record->barcode);
                    ...
                    if ($barcode_exists) {
                        ...
                        \Log::info('Adding stock step 2 for store '.$this->id.' with the itemcode '.$selected_record->itemcode.' and barcode '.$selected_record->id.' with the price '.$insert_arr['product_price'].' the distriputer price '.$insert_arr['distributor_price']);
                        \Log::info($stock);

                        if ($update_row) {
                            $updated = 1;
                            // update_base_product_store($selected_record->fk_product_id);
                        } else {
                            \Log::info('Bulk Stock Update From Server: Updating stock failed for the product store ID: '.$selected_record->id);
                        }
                        ...
                        if ($added_new_product_row) {
                            ...
                            \Log::info('Bulk Stock Update From Server: Adding new product failed for the product barcode: '.$barcode);
                        }
