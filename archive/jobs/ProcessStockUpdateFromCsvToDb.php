...
class ProcessStockUpdateFromCsvToDb implements ShouldQueue
{
    ...
    {
        \Log::info('ProcessStockUpdateFromCsv3 store '.$this->id.' with the fk_company_id '.$this->company_id.' - key '.$this->key);
        
        if (is_array($this->data)) {
            
            // Store the csv in the path
            // $stock_files_path = str_replace('\\', '/', storage_path("app/public/stock_files/"));
            // $filePath = $stock_files_path.'myCSVFile---------'.$this->key.'.csv';
            // $fp = fopen($filePath, 'w+');
